...
// |  4  (RST)      |    D7     | Display reset (active low)        |
// |  5  (CLK)      |    A4     | QSPI clock                        |
// |  6  (D1/IO2)   |    A3     | QSPI IO2                          |
// |  7  (CS)       |    D6     | Display chip select (active low)  |
// |  8 / 19 (GND)  |    GND    | Ground                            |
...
// |  GND           |    GND    | Ground          |
//
// The display QSPI bus (A0-A4 / PORTC) and the SD hardware SPI bus
...
//
// CLK is PC4.  The 0xF0 mask preserves PC4 (CLK) and PC5-PC7 while writing
// all four IO lines in a single register assignment.
//------------------------------------------------------------------------------
void qspiSendByte(uint8_t data)
  {
  uint8_t portval;

  // High nibble: data[7:4] -> PC[2,3,1,0]
  // portval  = ((data >> 4) & 0x03);         // bits 4,5 -> PC0, PC1
  // portval |= ((data >> 4) & 0x04) << 1;    // bit  6   -> PC3
  // portval |= ((data >> 4) & 0x08) >> 1;    // bit  7   -> PC2
  // PORTC = (PORTC & 0xF0) | portval;
  portval  = ((data >> 4) & 0x0f);         // bits 4,5 -> PC0, PC1
  PORTC = (PORTC & 0xF0) | portval;
  SET_CLK;
  CLR_CLK;

  // Low nibble: data[3:0] -> PC[2,3,1,0]
  // portval  = (data & 0x03);                // bits 0,1 -> PC0, PC1
  // portval |= (data & 0x04) << 1;           // bit  2   -> PC3
  // portval |= (data & 0x08) >> 1;           // bit  3   -> PC2
  // PORTC = (PORTC & 0xF0) | portval;
  portval  = (data & 0x0f);                // bits 0,1 -> PC0, PC1
  PORTC = (PORTC & 0xF0) | portval;
  SET_CLK;
  CLR_CLK;
  }
...
void writeCommand(uint8_t cmd)
  {
  CLR_CS;
  spiSendByte(0x02); spiSendByte(0x00);
  spiSendByte(cmd);  spiSendByte(0x00);
  SET_CS;
  }
...
void initDisplay()
  ...
  { uint8_t d[] = {0x58, 0x72}; writeCommandData(0xB1, d, 2); }

  // 2Ah (CASET): initialise full-width column window (0..367)
  { uint8_t d[] = {0x00, 0x00, 0x01, 0x6F}; writeCommandData(0x2A, d, 4); }

  // 2Bh (RASET): initialise full-height row window (0..447)
  { uint8_t d[] = {0x00, 0x00, 0x01, 0xBF}; writeCommandData(0x2B, d, 4); }
  ...
  writeCommandParam(0xF0, 0x50);
  { uint8_t d[] = {0x08, 0x08, 0x1E}; writeCommandData(0xB0, d, 3); }
  writeCommandParam(0xB4, 0x00);
  writeCommandParam(0xB5, 0x00);
  writeCommandParam(0xB6, 0xAE);
  { uint8_t d[] = {0x00, 0x00}; writeCommandData(0xB8, d, 2); }
  writeCommandParam(0xBE, 0x91);
  { uint8_t d[] = {0x01, 0x43, 0x01, 0x43}; writeCommandData(0xD6, d, 4); }
  { uint8_t d[] = {0x00, 0x50}; writeCommandData(0xB7, d, 2); }
  { uint8_t d[] = {0xAA, 0x88}; writeCommandData(0xBB, d, 2); }
  { uint8_t d[] = {0x44, 0x20, 0x10, 0x44, 0x20}; writeCommandData(0xBC, d, 5); }
  { uint8_t d[] = {0x06,0x00,0x23,0x00,0x02,0x17,0x04,0x17,
                   0x04,0x17,0x04,0x17,0x04,0x17,0x04,0x17};
    writeCommandData(0xC0, d, 16); }
  { uint8_t d[] = {0x56,0x34,0x12,0x65,0x43,0x21}; writeCommandData(0xC1, d, 6); }
  { uint8_t d[] = {0x65,0x43,0x21,0x56,0x34,0x12}; writeCommandData(0xC2, d, 6); }
  { uint8_t d[] = {0x36,0x63,0x25,0x52,0x14,0x41}; writeCommandData(0xC3, d, 6); }
  { uint8_t d[] = {0x63,0x36,0x52,0x25,0x41,0x14}; writeCommandData(0xC4, d, 6); }
  writeCommandParam(0xC5, 0x90);
  { uint8_t d[] = {0x06,0x00,0x23,0x00,0x02,0x17,0x04,0x17,
                   0x04,0x17,0x04,0x17,0x04,0x17,0x04,0x17};
    writeCommandData(0xC6, d, 16); }
  writeCommandParam(0xD0, 0x80);
  { uint8_t d[] = {0x43, 0x05}; writeCommandData(0xD8, d, 2); }

  // -----------------------------------------------------------------------
  // Page 0x51: Power supply, VCOM, and pixel amplifier settings
  // -----------------------------------------------------------------------
  writeCommandParam(0xF0, 0x51);
  { uint8_t d[] = {0x20, 0x20}; writeCommandData(0xB3, d, 2); }
  writeCommandParam(0xB4, 0x66);
  writeCommandParam(0xB5, 0x66);
  { uint8_t d[] = {0x47, 0x3B}; writeCommandData(0xB9, d, 2); }
  { uint8_t d[] = {0x13, 0x13, 0x13}; writeCommandData(0xBA, d, 3); }
  { uint8_t d[] = {0x83, 0x13}; writeCommandData(0xBB, d, 2); }
  { uint8_t d[] = {0x03, 0x03}; writeCommandData(0xBC, d, 2); }
  { uint8_t d[] = {0x15, 0x15}; writeCommandData(0xBD, d, 2); }
  writeCommandParam(0xBE, 0x10);
  writeCommandParam(0xC0, 0x1F);
  { uint8_t d[] = {0x30, 0x30, 0x30}; writeCommandData(0xC1, d, 3); }
  writeCommandParam(0xC2, 0x09);
  { uint8_t d[] = {0x07, 0xBB}; writeCommandData(0xC3, d, 2); }
  writeCommandParam(0xC4, 0x00);
  { uint8_t d[] = {0xFF, 0xFF, 0x01, 0x01}; writeCommandData(0xC9, d, 4); }
  writeCommandParam(0xCB, 0x23);
  { uint8_t d[] = {0x00,0x22,0x70,0x94,0x94,0x00,0x00}; writeCommandData(0xCC, d, 7); }
  { uint8_t d[] = {0x00,0x00,0x00,0x14}; writeCommandData(0xCD, d, 4); }
  { uint8_t d[] = {0x1E, 0x22}; writeCommandData(0xCF, d, 2); }
  { uint8_t d[] = {0x04,0x09,0x0E,0x09,0x0E,0x13,0x13,0x18,0x18,0x01};
    writeCommandData(0xD0, d, 10); }
  { uint8_t d[] = {0x0F, 0x2F}; writeCommandData(0xD1, d, 2); }
  { uint8_t d[] = {0x00,0x04,0x00,0x00,0x00,0x00,0x00}; writeCommandData(0xD2, d, 7); }
  { uint8_t d[] = {0x73, 0x65}; writeCommandData(0xD3, d, 2); }
  { uint8_t d[] = {0x2F,0x1F,0x1F,0x2F,0x1F,0x1F,0x1F,0x22,0x01};
    writeCommandData(0xD4, d, 9); }
  { uint8_t d[] = {0xA0,0x55,0x55,0x00}; writeCommandData(0xD6, d, 4); }
  { uint8_t d[] = {0xA0,0x55,0x55,0x00}; writeCommandData(0xD7, d, 4); }
  { uint8_t d[] = {0xA8,0xAA,0x55,0x00}; writeCommandData(0xD8, d, 4); }

  // -----------------------------------------------------------------------
  // Page 0x52: Gamma correction tables (R, G, B positive and negative curves)
  // -----------------------------------------------------------------------
  writeCommandParam(0xF0, 0x52);
  { uint8_t d[] = {0x01,0x4A,0xB5}; writeCommandData(0xB0, d, 3); }
  { uint8_t d[] = {0xAA,0xAE,0xDA,0xDA,0xDA}; writeCommandData(0xBA, d, 5); }
  { uint8_t d[] = {0x55,0x93,0xA7,0xB8,0xD8,0xA9,0xF4,0x0E,
                   0x27,0x3E,0xAA,0x55,0x6C,0x82,0x99};
    writeCommandData(0xB9, d, 15); }
  { uint8_t d[] = {0x00,0x00,0xD3,0xD7,0xE2,0x54,0xF1,0x0C,
                   0x23,0x37,0x55,0x48,0x58,0x66,0x7E};
    writeCommandData(0xB8, d, 15); }
  { uint8_t d[] = {0xAA,0x5D,0x84,0x84,0x84}; writeCommandData(0xB7, d, 5); }
  { uint8_t d[] = {0x55,0x60,0x72,0x82,0x9F,0x55,0xB9,0xD0,
                   0xE6,0xFB,0xAA,0x0F,0x24,0x36,0x4A};
    writeCommandData(0xB6, d, 15); }
  { uint8_t d[] = {0x00,0x00,0xCE,0xD0,0xD4,0x40,0xDB,0xEC,
                   0xFD,0x0E,0x55,0x1D,0x2B,0x37,0x4D};
    writeCommandData(0xB5, d, 15); }
  { uint8_t d[] = {0xAA,0x6F,0x9B,0x9B,0x9B}; writeCommandData(0xB4, d, 5); }
  { uint8_t d[] = {0x55,0x64,0x76,0x86,0xA4,0x95,0xBE,0xD7,
                   0xEE,0x04,0xAA,0x1A,0x30,0x45,0x5B};
    writeCommandData(0xB3, d, 15); }
  { uint8_t d[] = {0x00,0x00,0x9D,0xA2,0xB2,0x40,0xC3,0xE2,
                   0xF9,0x0D,0x55,0x1D,0x2B,0x38,0x4F};
    writeCommandData(0xB2, d, 15); }

  // -----------------------------------------------------------------------
  // Page 0x53: TCON (timing controller) sequencing
  // -----------------------------------------------------------------------
  writeCommandParam(0xF0, 0x53);
  { uint8_t d[] = {0x90,0x09,0x88,0x30,0x73,0x07,0x66,0x20,0x02};
    writeCommandData(0xC1, d, 9); }
  { uint8_t d[] = {0x22,0x30,0x03,0x44,0x00,0x77,0x60,0x06,0x55};
    writeCommandData(0xC2, d, 9); }
  { uint8_t d[] = {0xA3,0x00,0x00,0x05,0x00,0x00,0x00,0x00};
    writeCommandData(0xB1, d, 8); }
  { uint8_t d[] = {0x22,0x00,0x00,0x01,0x00,0x00,0xA7,0xA7};
    writeCommandData(0xB2, d, 8); }
  { uint8_t d[] = {0x23,0x80,0x00,0x01,0x00,0x3F,0x30};
    writeCommandData(0xB5, d, 7); }
  { uint8_t d[] = {0x21,0x80,0x00,0x01,0x00,0x3F,0x30};
    writeCommandData(0xB6, d, 7); }
  { uint8_t d[] = {0x22,0x80,0x00,0x00,0xA7,0x00,0x11};
    writeCommandData(0xB7, d, 7); }
  { uint8_t d[] = {0x21,0x80,0x00,0x00,0xA7,0x00,0x11};
    writeCommandData(0xB8, d, 7); }

  // -----------------------------------------------------------------------
  // Page 0x54: Source and gate driver output levels
  // -----------------------------------------------------------------------
  writeCommandParam(0xF0, 0x54);
  { uint8_t d[] = {0x00,0x00,0x00,0x00,0xC0,0x00,0x00,0x40,0x00};
    writeCommandData(0xB8, d, 9); }
  { uint8_t d[] = {0x00,0x40,0x00,0x00,0xC0,0x00,0x00,0x00,0x00};
    writeCommandData(0xB9, d, 9); }
  { uint8_t d[] = {0x00,0x20,0x00,0x00,0xC0,0x00,0x00,0x20,0x00};
    writeCommandData(0xBA, d, 9); }
  { uint8_t d[] = {0x00,0x00,0x00,0x00,0xC0,0x00,0x00,0x40,0x00};
    writeCommandData(0xBB, d, 9); }
  { uint8_t d[] = {0x00,0x40,0x00,0x00,0xC0,0x00,0x00,0x00,0x00};
    writeCommandData(0xBC, d, 9); }
  { uint8_t d[] = {0x00,0x20,0x00,0x00,0xC0,0x00,0x00,0x20,0x00};
    writeCommandData(0xBD, d, 9); }
  { uint8_t d[] = {0x20,0x04,0x02}; writeCommandData(0xBE, d, 3); }
  { uint8_t d[] = {0x12,0x14,0x00}; writeCommandData(0xBF, d, 3); }
  { uint8_t d[] = {0x00,0x24,0x02}; writeCommandData(0xC0, d, 3); }
  { uint8_t d[] = {0x22,0x04,0x00}; writeCommandData(0xC1, d, 3); }
  { uint8_t d[] = {0x10,0x14,0x02}; writeCommandData(0xC2, d, 3); }
  { uint8_t d[] = {0x02,0x24,0x00}; writeCommandData(0xC3, d, 3); }

  // -----------------------------------------------------------------------
  // Page 0x55: Additional display and power settings
  // -----------------------------------------------------------------------
  writeCommandParam(0xF0, 0x55);
  { uint8_t d[] = {0x00,0x00,0x61,0x00,0x62,0x00,0x11,0x5E,0x62,0xBF,0x01,0x00};
    writeCommandData(0xB0, d, 12); }
  { uint8_t d[] = {0x15, 0x09}; writeCommandData(0xB1, d, 2); }
  { uint8_t d[] = {0x10,0x43,0x65,0x87,0xA9,0xCB,0xDC,0xED,
                   0xEE,0xFF,0x1F,0x53,0x97,0xCB,0xFE,0xFF};
    writeCommandData(0xB2, d, 16); }
  { uint8_t d[] = {0xAD,0x58,0x03,0xDF,0x7B,0x04,0xCF,0x48,
                   0xF0,0x5A,0xF0,0x4A,0xEF,0x18,0xAF,0xF2};
    writeCommandData(0xB3, d, 16); }
  { uint8_t d[] = {0x3B,0xBF,0xF2,0x19,0x6E,0xCF,0xF2,0xF7,
                   0x2C,0x5E,0x9F,0xBF,0xD0,0xE2,0xE3,0xF4};
    writeCommandData(0xB4, d, 16); }
  { uint8_t d[] = {0xF4,0x00,0x00,0x00,0x00,0x00,0x00,0x00,
                   0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00};
    writeCommandData(0xB5, d, 16); }
  { uint8_t zeros[16] = {0}; writeCommandData(0xB6, zeros, 16); }
  { uint8_t zeros[16] = {0}; writeCommandData(0xB7, zeros, 16); }
  { uint8_t zeros[16] = {0}; writeCommandData(0xB8, zeros, 16); }
  { uint8_t zeros[12] = {0}; writeCommandData(0xB9, zeros, 12); }
...
#if (0 != LINES_DEMO)
#define mSwap(a, b, t)  { t = a; a = b; b = t; }
...
bool drawBitmapFromSD(const char *filename)
  ...
  Serial.print(F("SD: "));       Serial.print(filename);
  Serial.print(F("  "));         Serial.print(imgW);
  Serial.print(F("x"));          Serial.print(imgH < 0 ? -imgH : imgH);
  Serial.print(F("  "));         Serial.print(bpp);
  Serial.print(F("bpp  "));      Serial.println(imgH < 0 ? F("top-down") : F("bottom-up"));
...
void show_BMPs_in_root(void)
  ...
  while (1)
    ...
    bool is_bmp = !bmp_file.isDirectory()
               && (strstr(bmp_file.name(), ".BMP") != NULL
                || strstr(bmp_file.name(), ".bmp") != NULL);
...
void setup()
  ...
  DDRB |= PORTB_OUT_MASK;   // PB4, PB6, PB7 as outputs
  DDRC |= PORTC_OUT_MASK;   // PC0-PC4 as outputs for QSPI
  DDRD |= PORTD_OUT_MASK;   // PD4, PD6, PD7 as outputs
...
void loop()
  ...
  for (uint8_t cx = 0; cx < (DISP_WIDTH / 16); cx++)
    ...
    for (uint8_t cy = 0; cy < (DISP_HEIGHT / 16); cy++)
      ...
      if ((cx ^ cy) & 1)
        ...
        color = (uint16_t)((cx * 2) & 0x1F) << 11
              | (uint16_t)((cy * 2) & 0x3F) << 5
              | 0x1F;  // full blue component
  ...
  for (uint8_t i = 2; i < DISP_WIDTH / 2; i += 2)
    ...
    uint8_t gb = 0xFF - rb;
    uint16_t color = ((uint16_t)(rb >> 3) << 11)
                   | ((uint16_t)(gb >> 2) << 5)
                   | 0x1F;                       // full blue
